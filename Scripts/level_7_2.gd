...
func _ready() -> void:
	super()
	Global.time=399
	await get_tree().process_frame # Replace with function body.
	var red_cheep=[97,128,150,167,183,185]
	for enemy in $Enemies.get_children():
		var cellpos=$Enemies.local_to_map($Enemies.to_local(enemy.global_position)).x
		if cellpos in red_cheep:
			enemy.palette="red"
...
func _process(_delta: float) -> void:
	...
	if mario:
		if mario.global_position.x<5717.5:
