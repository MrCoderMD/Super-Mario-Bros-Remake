...
const invisible_blocks=[Vector2i(41,9)]
const powerupcoords=[]
const lifeupcoords=[]
...
func _ready() -> void:
	...
	if !Global.can_transition:
		mario.z_index=$Node2D/ForegroundTile.z_index-1
		mario.global_position=self.to_global($Node2D/ForegroundTile.map_to_local(Global.spawn[Global.currentlevel]))+Vector2(-16,mario.collision.shape.extents.y-16)
		await mario.slide_up()
		mario.z_index=$Node2D/ForegroundTile.z_index+1
	var hop_koopax=[30,32,46,48]
	for enemy in $Enemies.get_children():
		if $Enemies.local_to_map($Enemies.to_local(enemy.global_position.x)) in hop_koopax:
...
func _on_right_transition_body_entered(body: Node2D) -> void:
	if body.name=="Mario":
		Global.can_transition=true
		Global.transitionx["8-4"]=level.get_node("RightTransition").global_position.x# Replace with function body.\
		Global.transition_level["8-4"]=load("res://Scenes/level_8_4_3.tscn")
		Global.spawn["8-4"]=Vector2i(8,11)
